...
using System.IO;
using System.Text;
using System.Text.RegularExpressions; // StringBuilder
...
namespace HTMLConverter
{
    ...
    internal class HtmlParser
    {
        ...
        private XmlElement ParseHtmlContent()
        {
            ...
            while (_htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.EOF)
            {
                ...
                else if (_htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.ClosingTagStart)
                {
                    ...
                    if (_htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
                    {
                        htmlElementName = _htmlLexicalAnalyzer.NextToken.ToLower();
        ...
        private void AddTextContent(string textContent)
        {
            ...
            XmlElement htmlParent = _openedElements.Peek();
           prvioushtmlNmae =  htmlElementName;
            error = Regex.Matches(textContent, @"[a-zA-Z]").Count;
            if (error > 0)
            {
                h = 0;
                //if (textContent.Contains(" ") && textContent.Length > 0)
                //{
                //    textContent += "\n"; 
                //}
            }
            else if (h == 0 && textContent.Length == 1)
            {
                textContent += "\n";
                h = 1;
            }
        ...
        //private bool wenisfalse;
//private  int check;
private int h;
private string prvioushtmlNmae;
